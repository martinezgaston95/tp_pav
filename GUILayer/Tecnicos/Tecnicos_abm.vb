Public Class Tecnicos_abm
    ...
    Private Sub Tecnicos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Select Case _action
            ...
            Case Opcion.delete
                mostrar_datos()
                Me.Text = "Habilitar/Deshabilitar Tecnico"
    ...
    Friend Sub mostrar_datos()
        If Not IsNothing(_row_selected) Then
            ...
            cbo_sucursal.Text = _row_selected.Cells("col_suc").Value
            txt_fecha_ingreso.Text = _row_selected.Cells("col_fecha_ingreso").Value
            txt_fecha_nac.Text = _row_selected.Cells("col_fecha_nacim").Value
            ...
            Case Opcion.insert 'en el bdHelper hacemos BDHelper.getInstancia.EjecutarSql(sql_str)
                If existe_doc() = False Then
                    If validar_campos() Then 'cbo_perfil.SelectedValue.ToString seleccionamos el valor por detras del combo y lo transformamos en ToString  para que sean del mismo tipo antes y dsp del += me deje concatenar
                        str_sql = "INSERT INTO Tecnicos (n_tecnico,doc ,tipo_doc ,fecha_ingreso , estado , mail ,fecha_nacimiento , observaciones, tel_contacto , id_sucursal  )VALUES ('"
                        str_sql += txt_nombre.Text.ToString + "',"
                        str_sql += txt_nro_doc.Text.ToString + ","
                        str_sql += cbo_tipo_doc.SelectedValue.ToString + ",'"
                        str_sql += txt_fecha_ingreso.Text.ToString + "',"
                        str_sql += "'H','"
                        ...
                        str_sql += cbo_sucursal.SelectedValue.ToString + ")"

                        If BDHelper.getDBHelper.EjecutarSQL(str_sql) > 0 Then
                            MessageBox.Show("Tecnico añadido!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Me.Close()
                        End If
                    End If
                Else
                    MessageBox.Show("Tecnico ya existente!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If


            Case Opcion.update
                If validar_campos() Then
                    str_sql = "UPDATE Tecnicos SET n_tecnico= '"
                    str_sql += txt_nombre.Text.ToString + "', tipo_doc="
                    str_sql += cbo_tipo_doc.SelectedValue.ToString + ", doc='"
                    str_sql += txt_nro_doc.Text.ToString + "', fecha_ingreso='"
                    str_sql += txt_fecha_ingreso.Text.ToString + "', mail='"
                    ...
                    str_sql += txt_tel.Text.ToString + ",id_sucursal="
                    str_sql += cbo_sucursal.SelectedValue.ToString + " WHERE (id_tecnico="
                    str_sql += _row_selected.Cells("col_id").Value.ToString + ")"
                    If BDHelper.getDBHelper.EjecutarSQL(str_sql) > 0 Then
                        MessageBox.Show("Tecnico modificado!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Me.Dispose()
                    Else
                        MessageBox.Show("Error al actualizar el tecnico!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                End If


            Case Opcion.delete
                If MessageBox.Show("Seguro que desea habilitar/deshabilitar el tecnico seleccionado?", "Aviso", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.OK Then
                    estado = BDHelper.getDBHelper.ConsultaSQL("Select estado from Tecnicos where id_tecnico = " + _row_selected.Cells("col_id").Value).Rows(0).Item("estado")
                    If estado = "H" Then
                        estado = "D"
                    Else
                        estado = "H"
                    End If
                    str_sql = "UPDATE Tecnicos SET estado = '" + estado + "' WHERE id_tecnico = " + _row_selected.Cells("col_id").Value
                    If BDHelper.getDBHelper.EjecutarSQL(str_sql) > 0 Then
                        MessageBox.Show("Tecnico Habilitado/Deshabilitado!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Me.Dispose()
                    Else
                        MessageBox.Show("Error al actualizar el tecnico!", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
    ...
    Private Function validar_campos() As Boolean
        ...
        If cbo_sucursal.Text = String.Empty Then
            MessageBox.Show("Ingresar sucursal", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            cbo_sucursal.Focus()
            Return False
        Else
            cbo_sucursal.BackColor = Color.White
        End If

        If txt_fecha_ingreso.Text = String.Empty Then
            MessageBox.Show("Ingresar fecha de ingreso", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_fecha_ingreso.Focus()
            Return False
        Else
            txt_fecha_ingreso.BackColor = Color.White
        End If
        ...
        Return True
    End Function
    Public Sub seleccionar_tecnico(ByVal op As Opcion, ByVal row_selected As DataGridViewRow)
        _action = op
    ...
    Private Function existe_doc() As Boolean
        Return BDHelper.getDBHelper.ConsultaSQL("select * from Tecnicos where doc =" + txt_nro_doc.Text).Rows.Count > 0
    End Function
